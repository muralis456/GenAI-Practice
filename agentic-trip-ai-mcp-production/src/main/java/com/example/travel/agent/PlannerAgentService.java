...
public class PlannerAgentService {
    ...
    public Map<String, Object> plan(TravelState state) {
        ...
        // Dates are resolved deterministically before the planner runs. The planner LLM
        // must never overwrite the authoritative current-turn dates with a date copied
        // from conversation history or generated by the model. This is especially
        // important for live flight searches where a one-day-old date must not reach
        // the MCP provider.
        boolean currentTurnHasDateHint = TripSlotHeuristics.hasDateHint(state.userRequest())
        ...
        if (departure.isBefore(LocalDate.now()) && !currentTurnHasDateHint) {
            departure = LocalDate.now();
            if (returning.isBefore(departure)) {
                returning = departure.plusDays(5);
            }
        }
