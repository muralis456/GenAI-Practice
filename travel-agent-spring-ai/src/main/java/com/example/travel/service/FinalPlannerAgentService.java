...
public class FinalPlannerAgentService {
    ...
    public String buildFinalPlan(TravelRequest request, String selectedModel, String flightInsights, String travelInsights, String itinerary, String historyContext) {
        log.debug("Building final plan for userId={}, destination={}", request.getUserId(), request.getDestination());
        String contextInstruction = historyContext != null && !historyContext.isBlank()
                ? historyContext + "\nUse the above context to maintain continuity with previous plans and preferences.\n"
                : "";
        String finalPlan = chatClient.prompt()
                ...
                .user(contextInstruction + "Model=" + selectedModel + ", userId=" + request.getUserId() + ", departureCity=" + request.getDepartureCity() + ", destination=" + request.getDestination() + 
