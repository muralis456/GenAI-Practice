...
/**
 * Hotel agent: LLM decides whether to call {@link HotelSearchTool}, then structures results.
 */
@Service
public class HotelAgentService {
    ...
    public HotelSearchResult search(TravelState state) {
        ...
        log.info("Hotel agent (LLM-first) destination={} cheaper={}", destination, cheaper);

        String system = "You are the Hotel Agent. Decide if you need web hotel search. "
                + "Call HotelSearchTool only when you need fresh hotel data. "
                + "After tool results, return JSON only with shape "
                + "{\"hotels\":[{\"name\":\"\",\"area\":\"\",\"priceRange\":\"\",\"rating\":\"\",\"suitableFor\":\"\",\"notes\":\"\"}]}. "
                + "Never use placeholders like 'Not specified'. Prefer 2-4 real hotel names.";
        ...
        List<HotelOption> hotels = jsonSupport.read(content, HotelExtraction.class)
        ...
        if (hotels.isEmpty()) {
            ...
            fallback.setNotes("Hotel search did not return structured options; try modifying budget or style.");
            hotels = List.of(fallback);
        }
        return new HotelSearchResult(new ArrayList<>(hotels), List.of());
    }
